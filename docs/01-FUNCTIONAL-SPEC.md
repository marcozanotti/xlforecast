...
| FR-303 | M | Report empirical coverage per level per model, measured **out of calibration** — fold *k*'s coverage is evaluated against the band built from folds ≠ *k*, then averaged over folds — alongside nominal coverage. **A single panel figure, not split by intermittency class:** measured coverage is 0.807 on an intermittent panel against 0.809 on a Gaussian one, so the split carries no information. The split belongs on the **tails**, which differ sharply (FR-307b), and lives in `CalibrationRow` / `XLF_Diagnostics` block 3. |
...
| FR-307b | M | **Report both tails separately, split by intermittency class.** With symmetric additive bands on intermittent data, **0.00%** of violations fall in the lower tail and **15.62%** in the upper, against a roughly balanced 10.2% / 5.5% on Gaussian data — invisible in the coverage figure, which reads ≈nominal either way. Reported in `CalibrationRow` and `XLF_Diagnostics` block 3, and stated on the methodology page. |
| FR-307c | M | **One-sided miscoverage on count data is inherent, not a defect to be corrected.** An earlier draft of FR-307b claimed a model with one-sided miscoverage "is not well calibrated". That overreached, and the measurement which would have supported it says the opposite. Asymmetric conformal bands — the α/2 and 1−α/2 quantiles of the *signed* residual distribution — were built and measured: they balance the tails on smooth series (0.074 / 0.090 against 0.125 / 0.066) and **under-cover intermittent ones at 0.693 against a nominal 0.80**, because on zero-inflated counts the lower quantile lands above zero and excludes every zero, where the symmetric band's wider lower offset falls below zero, clips to zero, and covers them. `y = 0` is a point mass; no continuous interval handles it gracefully. **Decision: symmetric + clipped everywhere** — safer on the data persona P1 owns, and one procedure across the leaderboard, which is the commensurability ADR-006 protects. |
...
miscoverage exceeds 5%, against a Gaussian control where both tails are live. Neither assertion
treats the imbalance as a defect — FR-307c records why it is not.
